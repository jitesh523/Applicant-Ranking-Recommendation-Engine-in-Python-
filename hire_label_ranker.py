...
def build_features(df: pd.DataFrame):
    ...
    # Binary relevance: 1 if Hire, 0 if Reject
    y = (df["Recruiter Decision"].astype(str) == "Hire").astype(int).to_numpy()
...
def evaluate_binary_ranking(df, roles, scores, labels, k_list=(3, 5, 10)):
    ...
    def precision_at_k(relevances, k):
        rel = np.asarray(relevances)[:k]
        return float(np.sum(rel)) / max(1, len(rel))
    ...
    for role in unique_roles:
        ...
        role_labels_sorted = role_labels[order]

        for k in k_list:
            metrics[f"ndcg@{k}"].append(ndcg_at_k(role_labels_sorted, k))
            metrics[f"precision@{k}"].append(precision_at_k(role_labels_sorted, k))

        hits = np.where(role_labels_sorted == 1)[0]
        if hits.size > 0:
...
def main():
    ...
    summary = evaluate_binary_ranking(df.iloc[val_idx].copy(), roles_val, val_scores, y_val, k_list=(3, 5, 10))

    print("\n=== Ranking evaluation using Recruiter Decision (Hire=1, Reject=0) ===")
    for k in (3, 5, 10):
