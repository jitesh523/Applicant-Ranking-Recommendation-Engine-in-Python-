...
def build_features(df: pd.DataFrame):
    ...
    # Relevance from AI Score (0-100) to 0-4
...
def train_model(X_train, y_train, roles_train):
...
def fairness_report(df: pd.DataFrame, roles, scores, y, high_relevance_level=3):
    df = df.copy()
    df["model_score"] = scores
    df["relevance"] = y

    # Example groups by Education level
    groups = df["Education"].unique()

    print("=== Fairness proxy across Education groups ===")
    for g in groups:
        mask = df["Education"] == g
        sub = df[mask]
        if sub.empty:
            continue
        # Fraction of highly relevant candidates (label >= level)
        frac_high_label = np.mean(sub["relevance"] >= high_relevance_level)
        # Fraction of top-20 ranked (by model) that are from this group, within the whole dataset
        top20 = df.sort_values("model_score", ascending=False).head(20)
        frac_in_top20 = np.mean(top20["Education"] == g)
        print(f"Education={g}: frac_high_label={frac_high_label:.3f}, frac_in_top20={frac_in_top20:.3f}")
...
def main():
    ...
    model = train_model(X_train, y_train, roles_train)

    val_scores = model.predict(X_val)
    df_val = df.iloc[val_idx].copy()

    fairness_report(df_val, roles_val, val_scores, y_val, high_relevance_level=3)
